 ...
 * # open 的入参
 *
 * title 复用包内 Title 形态（string / Ref / getter）
 */
 ...
 * - title 的 Ref / getter 形态是活引用：源变化天然反映到求值结果，
 *   无需通知机制参与
 */
export function createIframeManager(maxCache: number): IframeManager {
   ...
   * # 快照投影
   *
   * trigger 表达「core 状态结构变更」，读取时向 core 取最新快照
   */
  ...
  return {
    list: project(() => core.getList()),
    openedList: project(() => core.getOpenedList()),
