 ...
 *
 * @description
 * - 与直接返回管理器的工厂不同：本工厂返回的 `useMobileAdaptation()`
 *   须在 setup 内调用——resize 监听的注册与清理依赖组件生命周期
 * - 数据所有权：只存储 `width` 一个观测量，`mode` 恒为 `resolveMode`
 *   的派生投影；未观测过宽度（创建后至首次 setWidth 前）按桌面宽度
 *   兜底，即创建时仅按 UA 判定（无 DOM 访问，SSR 安全）
 * - 监听按消费者计数：多个组件同时调用 useMobileAdaptation 时，
 *   随最后一个消费者卸载才移除
 */
